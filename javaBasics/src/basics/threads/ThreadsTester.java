...
public class ThreadsTester {
	...
	/**
	 * Launches more threads in parallel than the OS is allowed to.
	 * This method should end with an exception at runtime
	 */
...
//		launchMaxThreads();
	...
	/**
	 * Launches just one thread.
	 * Creates the content of the thread by using the Runnable FI, implementing the void run() method with a lambda expression. 
	 * () -> {return;} matches the run() abstract method of the Runnable FI.  
	 */
	private static void launchOneThread() {
		new Thread( () -> System.out.println("I am implementing the void run() abstract method of the Runnable FI.") ).start();
	}
}
